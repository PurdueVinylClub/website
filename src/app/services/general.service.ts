...
import { Credientals } from "../credentials.js";
...
export class GeneralService {
    ...
    getDiscogsClubCollection(callbackFunction: (data) => void) {
        let url = 'https://api.discogs.com/users/PurdueVinylClub/collection/folders/0/releases?token=' + Credientals.DISCOGS_AUTH_TOKEN;
    ...
    getGoogleCalendarEvents(callbackFunction: (data) => void ) {
        let url = 'https://www.googleapis.com/calendar/v3/calendars/' + Credientals.GOOGLE_CAL_CALENDAR_ID + '/events?key=' + Credientals.GOOGLE_CAL_API_KEY;
        this.httpClient.get(url).subscribe((result) => {
